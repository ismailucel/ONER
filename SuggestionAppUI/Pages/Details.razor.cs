     ...
     private async Task CompleteSetStatus()
     {
         switch (settingStatus)
         {
             case "completed":
                 ...
                 suggestion.OwnerNotes = $"You are right, this is an great idea for our developers. We created resource about it here: <a href='{urlText}' target='_blank'>{urlText}</a>";
                 break;
             case "watching":
                 suggestion.SuggestionStatus = statuses.Where(s => s.StatusName.ToLower() == settingStatus.ToLower()).First();
             ...
             case "Implementation":
                 suggestion.SuggestionStatus = statuses.Where(s => s.StatusName.ToLower() == settingStatus.ToLower()).First();
                 suggestion.OwnerNotes = "We are implementing this project now. ";
                 break;
             case "Design":
                 suggestion.SuggestionStatus = statuses.Where(s => s.StatusName.ToLower() == settingStatus.ToLower()).First();
                 suggestion.OwnerNotes = "We are designing this project now. ";
                 break;
             case "Analysis":
                 suggestion.SuggestionStatus = statuses.Where(s => s.StatusName.ToLower() == settingStatus.ToLower()).First();
                 suggestion.OwnerNotes = "We are analysing this project now. ";
                 break;
             case "Planing":
                 suggestion.SuggestionStatus = statuses.Where(s => s.StatusName.ToLower() == settingStatus.ToLower()).First();
                 suggestion.OwnerNotes = "This is a great idea. We are planing this project now. ";
                 break;
             case "Testing&Integration":
                 suggestion.SuggestionStatus = statuses.Where(s => s.StatusName.ToLower() == settingStatus.ToLower()).First();
                 suggestion.OwnerNotes = "We are making tests and integrations this project now. ";
                 break;
             default:
                 return;
         }
